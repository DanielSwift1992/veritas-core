...
Usage:
    $ python concat_repo.py                     # scan current dir → repo_concat.txt
    $ python concat_repo.py .. --out dump.md    # scan parent repo, custom output
    $ python concat_repo.py docs --limit 0      # concat only docs folder, no line limit
"""
...
def main() -> None:
    ...
    parser.add_argument("root", nargs="?", default=".", help="Root directory to scan (default: current working dir)")
    ...
    write_concat(root, Path(args.out).expanduser(), add_sha=args.sha, line_limit=args.limit, extra_patterns=args.exclude)
